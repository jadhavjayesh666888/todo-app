...
const COMMON_ICONS = [
  ...
  'Gamepad', 'ShoppingBag', 'CreditCard', 'Hash', 'PenTool', 'Compass', 'Archive', 'Film'
];
...
export default function TodoCategoryModal({ isOpen, onClose, categories = [], onSave }) {
  ...
  const saveCategory = () => {
    ...
    if (editingId) {
      setLocalCategories(localCategories.map(c => {
        if (c.id === editingId) {
          return { id: editingId, name: newCatName, color: newCatColor, icon: selectedIcon };
        }
        return c;
      }));
      setEditingId(null);
      ...
      <div className="glass modal-content animate-pop" style={{ maxWidth: '600px', width: '95%', textAlign: 'left', maxHeight: '90vh', overflowY: 'auto', padding: '2rem', borderRadius: '1.5rem', backgroundColor: 'var(--surface-card)', border: 'var(--glass-border)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800 }}>Manage Collections</h2>
          </div>
        ...
        <div className="glass" style={{ marginBottom: '2.5rem', background: 'rgba(255,255,255,0.03)', padding: '1.5rem', borderRadius: '1rem', border: '1px solid var(--border-color)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-disabled)', marginBottom: '0.5rem', letterSpacing: '0.05em' }}>Collection Name</label>
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                <input 
                  ...
                  placeholder="e.g., Movies, Study, Personal..." 
                  value={newCatName}
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-disabled)', marginBottom: '0.5rem', letterSpacing: '0.05em' }}>Color</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-disabled)', marginBottom: '0.5rem', letterSpacing: '0.05em' }}>Select Icon</label>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-secondary)' }}>Existing Collections</h3>
          {localCategories.map(cat => (
            <div key={cat.id} className="glass" style={{ padding: '1rem', borderRadius: '1rem', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'rgba(255,255,255,0.02)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', backgroundColor: `${cat.color}22`, color: cat.color, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <button onClick={() => removeCategory(cat.id)} style={{ background: 'transparent', border: 'none', color: 'var(--accent-danger)', opacity: 0.7, cursor: 'pointer', padding: '0.5rem' }}>
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
          ))}
          {localCategories.length === 0 && <div style={{ padding: '2rem', textAlign: 'center', opacity: 0.5, backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: '1rem' }}>No collections created yet.</div>}
        </div>

        <div style={{ marginTop: '2.5rem', display: 'flex', gap: '1rem' }}>
          <button onClick={onClose} className="btn btn-ghost" style={{ flex: 1, padding: '1rem', borderRadius: '1rem' }}>Cancel</button>
          <button onClick={handleSave} className="btn btn-primary" style={{ flex: 1, padding: '1rem', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', fontWeight: 800 }}>
